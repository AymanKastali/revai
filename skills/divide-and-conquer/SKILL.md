...
description: Use at the Decide gate of /revai:feature, /revai:bugfix, or /revai:refactor when the survey/spec shows the change is too big for one PR — or when designing-architecture is ordering a build. Judges whether the work should become a sequence of small, independently shippable PRs instead of one, and how to order that sequence. Does not own task/step granularity within a single PR — that's superpowers:writing-plans' job.
---

# Divide and conquer (splitting a big plan into a sequence of small PRs)
...
- Present the full slice sequence at the Decide gate, but scope *this run's* plan, diagnosis, or
  ...
  later `/revai:feature`, `/revai:bugfix`, or `/revai:refactor` invocation.
...
- Building the chosen slice still runs through the command's normal Build stage — TDD,
  `superpowers:subagent-driven-development`/`executing-plans` — this skill only decides slice
  boundaries and order, never how a slice gets built.
