...
description: Use when writing tests for backend code — APIs, services, or data access. Complements test-driven-development (the method) with backend-specific choices about what to test against: exercise the API/service boundary, use a real database over mocking it, keep data deterministic via factories, and isolate tests from each other.
---
