...
| Viscosity | Doing the right thing is harder than doing the wrong thing — the design itself invites a hack (design viscosity), or the build/test/review loop is so slow that cutting the corner is the path of least resistance (environment viscosity). | Fix whatever makes the correct approach the harder path — often the same fix as the smell it produces (a hidden dependency, an untested extraction point); if the drag is the environment itself (a slow build, a painful review process), fix that directly rather than treating it as a reason to keep hacking. |
...
| Long parameter list | A function or constructor takes so many arguments that call sites are unreadable and it's easy to pass two of the same type in the wrong order. | Group the arguments that travel together into one type, per `reference/functions.md`'s few-arguments rule (0-2 ideal, 3 avoided where possible). |
