...
`domain-driven-design` for the layers referenced below.

## What to test, by layer

| Layer | Test style | Assert on |
|---|---|---|
| **domain** — aggregates, value objects, domain services | pure unit, **no mocks** | invariants hold, valid state transitions, invalid input is rejected, domain events raised (see `domain-driven-design`) |
...
| **infra** — repositories, adapters | integration | mapping and round-trips, constraint violations — **for the how (real ephemeral DB, factories, isolation) use `reference/backend-testing.md`** |
...
code over mocks — is owned by `superpowers:test-driven-development` and `reference/backend-testing.md`.
The slant specific to this skill:
...
- [ ] Infra/integration testing follows `reference/backend-testing.md` (real ephemeral deps, not mocks)
