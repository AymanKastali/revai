...
traffic. (For schema changes, see `reference/safe-schema-changes.md`.) Most backend latency and
corruption bugs live here.
...
- **Context on every query.** Every DB call takes the caller's `context.Context` so its deadline and
  ...
  `reference/resilience-and-timeouts.md` owns the deadline/propagation policy.
...
- **Map "no rows" to a domain error.** A missing row is an expected outcome — translate it to a
  not-found domain error, not a generic failure (see `reference/error-handling-and-logging.md`).
