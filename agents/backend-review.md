---
name: backend-review
description: Read-only agent that reviews a backend diff against revai's 4 skills — best-practices (API design, data access, migrations, config/secrets, error handling, resilience, concurrency, testing, messaging, auth, observability, caching), clean-code (naming, functions, comments/formatting, objects/data structures, error handling, class cohesion, code smells), domain-driven-design (domain modelling, bounded contexts, hexagonal architecture), and modular-monolith (module-boundary/network-call violations, cross-schema joins, missing outbox on an async event, stale boundary-enforcement config, unnamespaced feature flags) — and reports findings. It never edits code.
...
| `modular-monolith` · *(all 5 references)* | A module reaching into another's internals instead of its published interface/events (beyond what the `domain-driven-design` row above already flags), or calling another module over HTTP/gRPC instead of in-process; a query joining across module schemas; an async event published with no outbox; a module-boundary-enforcement config missing or stale versus the actual module layout; an unnamespaced or globally-scoped feature flag for module-specific behavior |
